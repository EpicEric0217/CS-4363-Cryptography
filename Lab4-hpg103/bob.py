...
def main():
    ...
    key = ""
    host = "127.0.0.1"  # Enter correct IP address here
    port = 8080
    buf = 1024 * 2
    address = (host, port)
    ...
    # print("Shared secret between Bob and Alice is", sharedSecret)
    # welcome to the server message
    print("Waiting to receive messages...")

    # listening loop
    # Enter your code here to receive multiple messages from alice
    while True:
        inputdata, address = UDPSock.recvfrom(buf)
        inputdata = str(inputdata, 'utf-8')
        mssg = decrypt(inputdata, useClientPKI, useDHKey, sharedKey)
        if mssg == "exit":
            os._exit(0)
        print("Received secured message:", mssg)
